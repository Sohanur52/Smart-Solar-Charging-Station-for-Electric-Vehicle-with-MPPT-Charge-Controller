...
// NEW: Network Info Display Page
// NEW: Network Info + CPU Usage Display Page
// NEW: Network Info + CPU Usage Display Page
void displayNetworkInfo() {
  static unsigned long lastNetworkDisplay = 0;
  static int displayState = 0;
  static unsigned long idle0_prev = 0, idle1_prev = 0;
  static unsigned long lastSample = 0;
  
  // Variables to store current values (declare them outside if blocks)
  unsigned long idle0_current = 0;
  unsigned long idle1_current = 0;
  ...
  if (millis() - lastSample > 2000) {
    ...
    if (idle0 != NULL && idle1 != NULL) {
      ...
      // Get current idle times
      vTaskGetInfo(idle0, &task_status, pdTRUE, eInvalid);
      idle0_current = task_status.ulRunTimeCounter;
      
      vTaskGetInfo(idle1, &task_status, pdTRUE, eInvalid);
      idle1_current = task_status.ulRunTimeCounter;
      
      // Store for next calculation
      idle0_prev = idle0_current;
      idle1_prev = idle1_current;
    }
  }
  
  // Toggle display every 3 seconds
  if (millis() - lastNetworkDisplay > 3000) {
  ...
  // State 1: Hotspot Info
  else if (displayState == 1) {
    lcd.print("Hotspot: ESP_MPPT");
    lcd.setCursor(0, 1);
    lcd.print(WiFi.softAPIP().toString());
  }
  
  // State 2: CPU Usage (Both Cores)
  else {
    // Get current idle times for calculation
    TaskHandle_t idle0 = xTaskGetIdleTaskHandleForCPU(0);
    TaskHandle_t idle1 = xTaskGetIdleTaskHandleForCPU(1);
    ...
    if (idle0 != NULL && idle1 != NULL && idle0_prev > 0 && idle1_prev > 0) {
      TaskStatus_t task_status;
      ...
      unsigned long idle0_diff = idle0_current - idle0_prev;
      unsigned long idle1_diff = idle1_current - idle1_prev;
      
      // The runtime counter increments at configTICK_RATE_HZ (typically 1000Hz)
      // Over 2 seconds, that's 2000 ticks
      unsigned long total_ticks = 2000; // 2 seconds * 1000 Hz
      
      if (total_ticks > 0) {
        core0_usage = 100.0 - (idle0_diff * 100.0 / total_ticks);
        core1_usage = 100.0 - (idle1_diff * 100.0 / total_ticks);
      }
    }
    
    // Constrain to 0-100%
    core0_usage = constrain(core0_usage, 0, 100);
    core1_usage = constrain(core1_usage, 0, 100);
    
    lcd.print("Core0:");
    lcd.print(core0_usage, 1);
    lcd.print("%");
    
    lcd.setCursor(0, 1);
    lcd.print("Core1:");
    lcd.print(core1_usage, 1);
    lcd.print("%");
...
void LCD_Menu(){
  int 
  menuPages          = 5,  // CHANGED: Increased from 4 to 5 to add network page
  subMenuPages       = 12,
  longPressTime      = 3000,
  longPressInterval  = 100,
  shortPressInterval = 50;
  ...
  //MAIN MENU
    ...
    if(currentLCDMillis-prevLCDMillis>=millisLCDInterval&&enableLCD==1){   //Run routine every millisLCDInterval (ms)
      ...
      else if(buttonSelectCommand==1&&menuPage==5){buttonSelectCommand=0;settingMode=1;lcd.clear();} // CHANGED: Settings is now on page 5
      if(menuPage>menuPages){menuPage=0;}
      else if(menuPage<0){menuPage=menuPages;}  

      // DISPLAY PAGES - ADDED new page 4 for Network Info
      ...
      else if(menuPage==4){displayNetworkInfo();}  // NEW: Network info page
      else if(menuPage==5){displayConfig5();}      // Settings menu page (moved from page 4 to 5)
    }    
  }
}