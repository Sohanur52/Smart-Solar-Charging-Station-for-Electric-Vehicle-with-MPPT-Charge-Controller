...
void Read_Sensors(){
  ...
  // Calculate output current based on input (your original working code)
  ...
  if(buckEnable==0 && FLV==0 && OOV == 0){                
    currentMidPoint = ((CSI/avgCountCS)*1.3300);
  }
  
  // ORIGINAL POWER COMPUTATION (keep these as-is)
  ...
  // ========== ESP32 CONSUMPTION ADJUSTED CALCULATIONS ==========
  // This section calculates adjusted values including ESP32 power consumption
  
  float esp32Power = 4.0;  // ESP32 idle power consumption in Watts
  
  // Calculate adjusted input power (includes ESP32 consumption)
  powerInputAdjusted = powerInput + esp32Power;
  
  // Calculate adjusted input current based on conditions
  if (voltageInput >= 1.0 && voltageInput <= 5.0) {
    // USB-only mode - no solar power being harvested
    currentInputAdjusted = 0.0;
    powerInputAdjusted = esp32Power;  // Only ESP32 power consumption
  }
  else if (voltageOutput > voltageInput) {
    // Battery is discharging (voltage higher than input) - no solar power being harvested
    currentInputAdjusted = 0.0;
    powerInputAdjusted = esp32Power;  // Only ESP32 power consumption
  }
  else {
    // Normal operation - solar power is being harvested
    if (voltageInput > 0) {
      currentInputAdjusted = powerInputAdjusted / voltageInput;
    } else {
      currentInputAdjusted = 0;
    }
    // powerInputAdjusted already set to powerInput + esp32Power
  }
  // ========== END OF ESP32 ADJUSTED CALCULATIONS ==========
  ...
  if (voltageInput >= 1.0 && voltageInput <= 3.0) {
    Serial.print("(USB Mode) ");
  } else if (voltageOutput > voltageInput) {
